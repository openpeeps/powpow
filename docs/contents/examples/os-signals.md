...
description: "SIGINT/SIGTERM/SIGHUP delivered through the loop, not global handlers."
keywords: ["powpow", "example", "os_signals"]
---

# Graceful shutdown signals

The classic production shutdown flow. OS signals arrive as loop events via a `SignalRelay`; the SIGINT/SIGTERM handlers stop the loop, after which the server and loop close cleanly. SIGHUP doubles as a reload hook.

Source: [`examples/os_signals.nim`](../../examples/os_signals.nim)
...
```bash
kill -TERM $(pgrep os_signals)
# or just press Ctrl+C
```

## How it works

- `newOsSignalRelay(loop)` + `relay.listen(SignalInt) do ()` wire signal numbers to callbacks.
- `relay.watchOsSignals([SignalInt, SignalTerm, SignalHup])` installs the plumbing once for all listed signals.
- Cleanup after `loop.run()` returns is safe: the loop is stopped, so `server.close(); loop.close()` cannot race callbacks.

[Signals guide](../core/signals.md) and [signal API](../api/signal.md).
