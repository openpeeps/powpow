---
title: Signal bus (pub/sub)
description: "In-process SignalRelay with listen, listenOnce and unlisten."
keywords: ["powpow", "example", "signal_bus"]
---

# Signal bus (pub/sub)

An HTTP endpoint emits numbered events on an internal `SignalRelay`; subscribers react on the same loop. Shows permanent listeners, auto-unsubscribing `listenOnce`, and manual `unlisten` after N deliveries.

Source: [`examples/signal_bus.nim`](../../examples/signal_bus.nim)
...
curl "http://localhost:9005/emit?signal=3"   # once-listener consumes it
curl "http://localhost:9005/emit?signal=3"   # nobody listens anymore
```

## How it works

- `newSignalRelay(loop, capacity)` allocates ids 0..capacity-1; emit is `relay.emit(n)`.
- `relay.listen(id, cb)` returns a `ListenerHandle`; call `.unlisten()` on it to detach explicitly.
- `listenOnce` detaches itself after the first delivery, handy for one-shot coordination.
- Everything stays on one loop, so listener callbacks need no synchronization.

[signal API](../api/signal.md).
