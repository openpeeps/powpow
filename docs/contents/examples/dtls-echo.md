---
title: DTLS echo server
description: "Encrypted UDP: DTLS 1.2 sessions, cookie exchange, MTU-sized sends."
keywords: ["powpow", "example", "dtls_echo"]
---

# DTLS echo server

DTLS 1.2 over the UDP backend with an embedded self-signed certificate. The server keeps one encrypted session per peer behind a single socket, enforces the stateless HelloVerifyRequest cookie exchange, and echoes payloads. The `--client` handshakes, sends a ping, then verifies an 8192-byte payload round trip, proving transparent splitting across MTU-sized datagrams.

Source: [`examples/dtls_echo.nim`](../../examples/dtls_echo.nim)
...
```bash
nim c -r examples/dtls_echo.nim            # server
nim c -r examples/dtls_echo.nim -- --client # in another terminal
```

## How it works

- `newServerDtlsContext(certPath, keyPath)` and `newDtlsServer(loop, addr, port, ctx, onData=..., onHandshakeDone=...)` mirror the TCP/TLS APIs.
- `sess.peerAddrStr()` and `srv.sessionCount()` expose per-peer identity and live session totals.
- Client side: `newClientDtlsContext(verifyPeer = false)` accepts the self-signed test cert; `connectDtls` starts the handshake.
- Large `sess.send(payload)` calls split into DTLS records sized to the path MTU and reassemble into `onData` on the receiver.
- POSIX only, like TLS.

[DTLS guide](../net/dtls.md).
