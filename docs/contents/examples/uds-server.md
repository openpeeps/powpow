---
title: HTTP over a Unix domain socket
description: "listenUnix serves HTTP on a UDS for low-latency local IPC."
keywords: ["powpow", "example", "uds_server"]
---

# HTTP over a Unix domain socket

Runs the same HTTP stack over a Unix domain socket instead of TCP, skipping the loopback stack entirely. Ideal for microservices on one machine. POSIX only; the whole example is guarded by `when not defined(windows)`.

Source: [`examples/uds_server.nim`](../../examples/uds_server.nim)
...
```bash
curl --unix-socket /tmp/powpow.sock http://localhost/hello
curl --unix-socket /tmp/powpow.sock http://localhost/echo -d 'ping'
```

## How it works

- A stale socket file from a previous run is removed before binding.
- `server.listenUnix(SockPath)` replaces `listen(host, port)`; everything else is identical TCP-flavored HTTP.
- This example drives the loop manually with `loop.run()` instead of `server.start()`, showing the lower-level startup path.

The client side speaks UDS too: pass `unixSocket=` to any request, see [httpclient](../api/httpclient.md).
