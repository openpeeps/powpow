...
description: "Bidirectional byte forwarding with connect buffering and paired teardown."
keywords: ["powpow", "example", "tcp_proxy"]
---

# TCP reverse proxy

A reverse proxy that forwards clients on :9020 to an echo backend on :9021. Bytes flow both directions; either side closing tears the pair down. Client bytes arriving before the upstream dials are buffered and flushed on connect, covering the async race window.

Source: [`examples/tcp_proxy.nim`](../../examples/tcp_proxy.nim)
...
```bash
nc 127.0.0.1 9020   # type; the backend echoes it back
```

## How it works

- Two `TcpServer`s share one loop; pairs are tracked in tables keyed by client fd, with a reverse table for upstream fds.
- `ProxyPair.pending` holds early client data until `onConnect` fires upstream.
- `teardown(fd)` closes whichever half survives and forgets both mappings; `state != Closed` guards double-closes.
- Pattern generalizes directly to load balancing: rotate the upstream address in `onAccept`.

[TCP guide](../net/tcp.md).
