---
title: WebSocket over HTTP
description: "websocketUpgrade shares one port between HTTP routes and a WS endpoint."
keywords: ["powpow", "example", "wsupgrade"]
---

# WebSocket over HTTP

Runs HTTP and WebSocket on port 9000 simultaneously: the `/ws` route calls `websocketUpgrade`, which completes the upgrade inline and hands back a live `WsConnection`. Everything else keeps serving normal HTTP, including a browser test page.

Source: [`examples/wsupgrade.nim`](../../examples/wsupgrade.nim)
...
- `websocketUpgrade(res, req, server, onOpen=..., onMessage=..., onClose=..., onError=...)` performs the handshake and registers the session in one call.
- After upgrading, the route should not touch `res` again; the connection now belongs to the WS layer.
- Same callback shape as the standalone server, so application code ports between both modes unchanged.
