...
description: "One event-loop thread per CPU core sharing one port via SO_REUSEPORT."
keywords: ["powpow", "example", "httpserver_threads"]
---

# Multi-threaded HTTP server

Scales the previous example horizontally: `newHttpServer()` spawns one worker thread per CPU core (pass an explicit count to override), each running its own event loop bound to the same port with `SO_REUSEPORT`. The kernel load-balances accepted connections across loops.

Source: [`examples/httpserver_threads.nim`](../../examples/httpserver_threads.nim)
...
- Requires `--threads:on`; the thread count defaults to `countProcessors()` or pass `newHttpServer(4)`.
- The handler must be `{.gcsafe.}` because it runs on several threads concurrently; do not share mutable state across requests without locks.
- Each connection lives entirely on the loop that accepted it, so per-request state stays single-threaded even though the server is not.
- Routing logic is identical to the single-threaded example; only the server constructor changes.
