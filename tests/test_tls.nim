## tests/test_tls.nim — TLS tests for powpow.
##
## Tests: implicit-TLS server (wrap on accept) echo and a STARTTLS-style
## in-place upgrade on an existing plaintext connection.

import ../src/powpow
import std/[unittest, os, strutils]

const TestCert = """-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
"""

const TestKey = """-----BEGIN PRIVATE KEY-----

K41fk5DdTExX/C2iR5wWzVbN
-----END PRIVATE KEY-----
"""

proc writeTestCert(): tuple[cert, key: string] =
  let dir = getTempDir() / "powpow-tls-test"
  discard existsOrCreateDir(dir)
  result.cert = dir / "test-cert.pem"
  result.key = dir / "test-key.pem"
  writeFile(result.cert, TestCert)
  writeFile(result.key, TestKey)

when not defined(windows):
  test "tls_implicit_server_echo":
    let (cert, key) = writeTestCert()
    let serverCtx = newServerTlsContext(cert, key)
    let loop = newLoop()
    var gotEcho = false
    var received = ""

    let server = newTcpServer(loop,
      onAccept = proc(conn: Connection) =
        conn.wrapTls(serverCtx)
      ,
      onData = proc(conn: Connection, data: openArray[byte]) =
        discard conn.send(data)
      ,
    )
    server.listen("127.0.0.1", 29876)

    discard loop.addTimer(50) do (id: int):
      let clientCtx = newClientTlsContext()
      loop.connect("127.0.0.1", 29876,
        onConnect = proc(conn: Connection) =
          conn.wrapTls(clientCtx)
          discard conn.send("hello powpow tls")
        ,
        onData = proc(conn: Connection, data: openArray[byte]) =
          received = cast[string](@data)
          gotEcho = true
          conn.close()
          server.close()
          loop.stop()
        ,
      )

    discard loop.addTimer(3000) do (id: int):
      server.close()
      loop.stop()

    loop.run()

    doAssert gotEcho, "TLS echo should have completed"
    doAssert received == "hello powpow tls", "echo mismatch: " & received
    loop.close()

  test "tls_starttls_upgrade":
    let (cert, key) = writeTestCert()
    let serverCtx = newServerTlsContext(cert, key)
    let loop = newLoop()
    var gotEcho = false
    var received = ""

    let server = newTcpServer(loop,
      onData = proc(conn: Connection, data: openArray[byte]) =
        if conn.isTlsActive():
          discard conn.send(data)
        else:
          let s = cast[string](@data)
          if s.contains("STARTTLS"):
            discard conn.send("250 OK\r\n")
            conn.wrapTls(serverCtx)
      ,
    )
    server.listen("127.0.0.1", 29877)

    discard loop.addTimer(50) do (id: int):
      let clientCtx = newClientTlsContext()
      var upgraded = false
      loop.connect("127.0.0.1", 29877,
        onConnect = proc(conn: Connection) =
          discard conn.send("STARTTLS\r\n")
        ,
        onData = proc(conn: Connection, data: openArray[byte]) =
          if not upgraded:
            upgraded = true
            conn.wrapTls(clientCtx)
            discard conn.send("hello after tls")
          else:
            received = cast[string](@data)
            gotEcho = true
            conn.close()
            server.close()
            loop.stop()
        ,
      )

    discard loop.addTimer(3000) do (id: int):
      server.close()
      loop.stop()

    loop.run()

    doAssert gotEcho, "STARTTLS upgrade echo should have completed"
    doAssert received == "hello after tls", "echo mismatch: " & received
    loop.close()
